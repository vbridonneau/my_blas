...
void my_dgemm(const enum CBLAS_ORDER Order, const enum CBLAS_TRANSPOSE TransA, const enum CBLAS_TRANSPOSE TransB, const int M, const int N, const int K, const double alpha, const double *A, const int lda, const double *B, const int ldb, const double beta, double *C, const int ldc) {
    ...
    // int mbloc = (int)floor((double)M/(double)BLOCK_SIZE), lastm = M % BLOCK_SIZE;
    // int nbloc = (int)floor((double)N/(double)BLOCK_SIZE), lastn = N % BLOCK_SIZE;
    for(int k = 0; k < kbloc; ++k) {
        for (int i = 0; i < kbloc; ++i) {
            for (int j = 0; j < kbloc; ++j) {
                int k_blk_size = (k < kbloc - 1) ? BLOCK_SIZE : lastk;
                int i_blk_size = (i < kbloc - 1) ? BLOCK_SIZE : lastk;
                int j_blk_size = (j < kbloc - 1) ? BLOCK_SIZE : lastk;
