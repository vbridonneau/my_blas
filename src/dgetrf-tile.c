...
void my_dgetrf_tile(const CBLAS_LAYOUT Order, int m, int n, double** a, int lda ) {
  ...
  for(j = 0; j < min( m, n ); j += nb) {
    jb = min( min( m, n ) - j, nb );
    my_dgetrf( Order, m-j, jb, a + j + j*lda, lda);

    if( j+jb < n ) {
      my_dtrsm( Order, CblasLeft, CblasLower, CblasNoTrans, CblasUnit, jb, n-j-jb, 1., a + j*(1 + lda), lda, a + j +(j+jb)*lda, lda );
      if( j+jb < m ) {
  	my_dgemm_scal_openmp( Order, CblasNoTrans, CblasNoTrans, m-j-jb,n-j-jb, jb, -1., a + j+jb + j*lda, lda, a + j + (j+jb)*lda, lda, 1., a + j + jb + (j+jb)*lda, lda );
      }
    }
  }
}
